...
function HomePageContent() {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records
      .filter((record) => {
        ...
        const matchesFee =
          feeFilter === "All" || record.feeStatus === feeFilter;
  ...
  const handleDelete = async () => {
    try {
      const res = await fetch(`/api/tuitions?id=${deleteId}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || "Failed");
          ...
          <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="text-sm text-slate-400">
                Manage bookings, salaries, and fee collection from one place.
              </p>
            </div>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
              <div className="relative w-full sm:w-auto flex items-center gap-2">
                <p className="text-sm text-slate-400 w-full">Fee Status</p>
                <select
                  value={feeFilter}
          ...
          {loading ? (
            <div className="flex items-center justify-center rounded-2xl border border-dashed border-white/10 bg-slate-800/50 py-12">
              <div className="flex items-center gap-2 text-slate-300">
                <Loader2 className="h-5 w-5 animate-spin" /> Loading tuition
                records...
              </div>
            </div>
          ) : (
            <div className="max-h-[100vh] overflow-auto rounded-2xl border border-white/10">
              <table className="min-w-[1080px] md:min-w-[1080px] lg:min-w-full divide-y divide-white/10 text-sm">
                <thead className="sticky top-0 z-10 bg-slate-900/95 backdrop-blur">
                  <tr className="text-left text-slate-400">
                    ...
                    <th className="px-3 py-3 font-medium">Booking Date</th>
                    <th className="px-3 py-3 font-medium">Fee Expiry</th>
                    ...
                    filteredRecords.map((record) => (
                      ...
                      >
                        ...
                        <td className="px-3 py-3">
                          <span
                            className={`rounded-full px-2.5 py-1 text-xs font-medium ${feeStyles[record.feeStatus] || feeStyles.Pending}`}
                          >
                            {record.feeStatus}
                          </span>
                        </td>
                        <td className="px-3 py-3">
                          <span
                            className={`rounded-full px-2.5 py-1 text-xs font-medium ${badgeStyles[record.bookingStatus] || badgeStyles.Pending}`}
                          >
                        ...
                        <td className="px-3 py-3 text-slate-200">
                          TK {Number(record.salary).toLocaleString()}
                        </td>
                        <td className="px-3 py-3 text-slate-200">
                          TK {Number(record.agencyFee).toLocaleString()}
                        </td>
            ...
            <div className="mb-5 flex items-center justify-between gap-3">
              <div>
                <h3 className="text-xl font-semibold">
                  {editingRecord ? "Edit Tuition" : "Add New Tuition"}
                </h3>
                <p className="text-sm text-slate-400">
                  Fill in the tuition details below.
                </p>
              </div>
              <button
                onClick={() => {
              ...
              >
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm({ ...form, agencyFee: e.target.value })
                    }
                    className="w-full rounded-2xl border border-white/10 bg-slate-800/70 px-3 py-2.5 text-sm outline-none focus:border-brand-500"
                  />
                </div>
